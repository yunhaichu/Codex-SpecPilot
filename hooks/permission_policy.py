...
def is_allowed_for_codex_worker(path, project_spec_text):
     ...
     # 2. Supervision files — always deny
     ...
     if mode == "supervised_project_development":
         ...
         if not project_spec_text or "wikiguard_self_development" not in project_spec_text:
     ...
     if mode == "wikiguard_self_development":
         ...
         # Still deny supervision files
         if is_hook_state_file(normalized):
             return (False, "hook state file protected even in self-dev mode")

     # Unknown mode: deny by default for non-obvious business files
     if mode == "unknown" or not project_spec_text:
         # Allow if it looks like a regular business file (source code, docs, etc.)
         if not normalized.startswith("."):
             return (True, "unknown mode, allowing non-hidden file (no Allowed Scope defined)")
         return (False, "unknown mode, no Allowed Scope defined, deny hidden files")

     # 4. Check ALLOWED scope patterns in project spec
     allowed_patterns = [
          r"allowed\s*scope",
          r"允许\s*修改",
     ...
     for pat in allowed_patterns:
         if re.search(pat, project_spec_text, re.IGNORECASE):
             # If we can't parse Allowed Scope precisely, lean toward allowing
             # business-looking files
             if not normalized.startswith(".") and not normalized.startswith("_"):
                 return (True, "no explicit deny, business file allowed")

     # Default: deny for non-obvious files
...
def is_allowed_for_hook_writer(hook_name, path):
     ...
     if hook_name == "PreToolUse":
         # Can only write guard_log.jsonl
...
def extract_paths_from_command(command):
     """从简单 Bash 命令中提取可能的路径."""
     paths = []
     # Simple heuristic: match anything that looks like a file path
     patterns = [
          r">\s*(\S+)",   # redirect target
          r">\s*>\s*(\S+)",
          r">>(\s*)(\S+)",
          r"\brm\s+(?:-\S+\s+)*(\S+)",
          r"\bmv\s+\S+\s+(\S+)",
